...
if (empty($termo)):
    ...
    $sql = 'SELECT doa_id, doa_nome, doa_nomedoador, doa_tipo, doa_valor FROM doacoes order by doa_nome';
    $stm = $conexao->prepare($sql);
    $stm->execute();
    $doacoes = $stm->fetchAll(PDO::FETCH_OBJ);

else:

    //Executa uma consulta baseada no termo de pesquisa passado como parametro
    $conexao = conexao::getInstance();
    $sql = 'SELECT doa_id, doa_nome, doa_nomedoador, doa_tipo, doa_valor FROM doacoes WHERE pro_nome LiKE :pro_nome OR doa_nomedoador LIKE :doa_nomedoador';
    $stm = $conexao->prepare($sql);
    $stm->bindValue(':doa_nome', $termo . '%');
    $stm->bindValue(':doa_nomedoador', $termo . '%');
    $stm->execute();
    $doacoes = $stm->fetchAll(PDO::FETCH_OBJ);
    ...
    <div class="container">
        <fieldset>
            ...
            <form action="" method="get" id="form-contato" class="">
                ...
                <div class="">
                    <input type="text" class="form-control" id="termo" name="termo" placeholder="informe o nome ou nome do doador " style="border-color: #000000; width: 100%;">
                </div>
                <div style="width: 100%; display: flex; flex-direction: row; justify-content: space-between;">
                    <button type="submit" class="btn btn-primary">Pesquisar</button>
                    <a href="indexdoa.php" class="btn btn-primary">Ver Todos</a>
                </div>

            </form>
            <div style="width: 100%; display: flex; flex-direction: row; justify-content: space-between;">
                <!-- Link para pagina de cadastro -->
                <a href="cadastrodoacao.php" class="btn btn-success">Cadastrar Doações</a>
            ...
            <?php if (!empty($doacoes)): ?>
                ...
                <div id="conteudoParaImprimir" class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <?php foreach ($doacoes as $doacao): ?>
                            <tr>
                                <td><?= $doacao->doa_nome ?></td>
                                <td><?= $doacao->doa_nomedoador ?></td>
                                <td><?= $doacao->doa_tipo ?></td>
                                <td><?= $doacao->doa_valor ?></td>
                                <td>
                                    <a href="editardoacao.php?id=<?= $doacao->doa_id ?>" class="btn btn-primary">Editar</a>
                                    <a href="javascript:void(0)" class="btn btn-danger link_exclusao" rel="<?= $doacao->doa_id ?>">Excluir</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                ...
                <?php else: ?>
                    <!-- Mensagem caso nao exista Doações ou nao encontrado -->
                    <h3 class="text-center text-primary">Não existem Doações cadastrados!</h3>
                <?php endif; ?>
                </div>
        </fieldset>
